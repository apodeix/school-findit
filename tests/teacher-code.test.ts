...
import {
  ...
  matchesTeacherCode,
  newTeacherCode,
} from "../lib/server/teacher-code.ts";
test("새 교사 코드는 짧거나 단순한 조합을 거부한다", () => {
  for (const code of [
    ...
    assert.equal(newTeacherCode.safeParse(code).success, false);
  assert.equal(newTeacherCode.safeParse("Example-7z!K2v9").success, true);
});
test("같은 코드도 서로 다른 salt/hash로 저장되며 원문은 저장하지 않는다", () => {
  const code = "Example-7z!K2v9",
    a = hashTeacherCode(code),
    b = hashTeacherCode(code);
